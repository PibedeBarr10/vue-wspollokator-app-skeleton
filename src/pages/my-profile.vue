<template>
  <FirstConfigurationModal :toDoForUser="toDoForUser" />
  <div
  ...
  >
      ...
      <div class="lg:w-3/5 lg:h-2/5 mx-auto flex flex-wrap">
        ...
        <div class="lg:w-2/5 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
          ...
          <div class="mb-4">
            <div class="form-control w-full">
              ...
              <select
                v-model="profile.accepts_animals"
        ...
        >
          <img
            ...
            :src="profile.avatar"
          />
          <h2 class="text-gray-900 text-xl font-medium mt-4">
            {{ currentUser.user.first_name }} {{ currentUser.user.last_name }}
          </h2>
          <h6 class="text-gray-500 text-2xs mb-2">
            {{ currentUser.user.email }}
          </h6>

          <label
            for="modal-change-picture"
            class="my-1 btn btn-primary modal-button w-full"
            >Zmień zdjęcie</label
          >
          <input
            type="checkbox"
            id="modal-change-picture"
            class="modal-toggle"
          />
          ...
          <button
            :class="[
              profileDataChanged
                ? 'btn btn-primary w-full my-1 border-0 py-2 px-4 btn-disabled'
                : 'btn btn-primary w-full my-1 border-0 py-2 px-4',
            ]"
    ...
    <AllOpinions v-if="dataLoaded" :user-id="this.currentUser.user.pk" />
  </div>
</template>
  ...
  methods: {
        ...
        .then((data) => {
          this.$store.dispatch("getProfile", {
            ...
            .then(() => {
              ...
              this.profile.preferable_price = parseInt(
    ...
    updateProfileData() {
      this.$store
        ...
        .then((response) => {
          this.$store.dispatch("notificationModule/show", {
            text: "Pomyślnie zaktualizowano dane profilu",
            type: "success",
          });
          this.profileDataChanged = true;
        })
    ...
    uploadAvatar() {
      if (!this.isFileImage(this.newAvatar)) {
        this.$store.dispatch("notificationModule/show", {
          text: "Wybrany plik nie jest obrazem",
          type: "error",
        });
        return;
      }
      ...
      this.$store
        ...
        .then(() => {
          this.$store.dispatch("notificationModule/show", {
            text: "Pomyślnie zaktualizowano zdjęcie",
            type: "success",
          });
        })
        .catch((error) => {
          this.$store.dispatch("notificationModule/show", {
            text: "Błąd w trakcie aktualizacji zdjęcia",
            type: "error",
          });
        });
    },
