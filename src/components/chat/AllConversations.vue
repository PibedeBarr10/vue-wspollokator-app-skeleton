...
<div class="flex flex-col">
  <div v-for="conversation in conversationsList" :key="conversation" >
    <div v-if="conversation.is_group">
           <div  class="flex m-2 bg-white rounded-lg p-2 cursor-pointer btn-ghost" @click="chooseGroupConversation(conversation.id,conversation.users,conversation.name)" >
                  <div class="flex-grow p-3">
                    <div class="flex justify-between text-xs">
                      <div>{{conversation.name}}</div>
                      <div v-if="conversation.last_message.text !==''" class=" text-gray-400">{{ moment(conversation.last_message.created_at) }}</div>
                    </div>
